...
for (i in 1:nrow(data)){
  if(data[i,"TH"] == 'yes' & data[i,"CM"] == 'yes'){
...
```{r}
compteur <- integer(nrow(Data_AMC_test))

# Initialiser un dictionnaire pour suivre le nombre d'occurrences de chaque identifiant
occurrences <- list()

# Parcourir les identifiants et mettre à jour les compteurs
for (i in 1:nrow(Data_AMC_test)) {
  id <- Data_AMC_test$ID[i]
  if (is.null(occurrences[[as.character(id)]])) {
    occurrences[[as.character(id)]] <- 1
  } else {
    occurrences[[as.character(id)]] <- occurrences[[as.character(id)]] + 1
  }
  compteur[i] <- occurrences[[as.character(id)]]
}

# Ajouter la colonne de compteurs au dataframe
Data_AMC_test$compteur <- compteur
...
```{r}
print(levels(Data_AMC_test$Douleur_TM))
```
```{r}
print(levels(Data_AMC_test$Douleur_Mobilite))
```
```{r}
print(levels(Data_AMC_test$Douleur_Nutrition))
```
```{r}
print(levels(Data_AMC_test$Douleur_Stress))
```
```{r}
print(levels(Data_AMC_test$Douleur_Phyto))
```
```{r}
print(levels(Data_AMC_test$Douleur_Hypnose))
```
```{r}
print(levels(Data_AMC_test$Douleur_Acup))
```
```{r}
print(levels(Data_AMC_test$Douleur_Bouillotte))
```
```{r}
print(levels(Data_AMC_test$Douleur_CBD))
```
```{r}
print(levels(Data_AMC_test$Douleur_Tens))
```
...
### ACM de tous les combos et habillages selon les effets secondaires (ES) ###
...
colonnes_combos_TM <- which(seq_along(Data_AMC_TM) %in% c(70, 71, 73, 74, 76, 77, 79, 80, 82, 85, 86, 88, 89, 91, 92, 94, 95, 97, 98)) #83 retiree CM_Hyp car une seule modalite

# Variables utilisees pour l'AMC avec les quali sup
colonnes_combos_TM_ES <- which(seq_along(Data_AMC_TM) %in% c(11, 14, 17, 20, 23, 26, 29, 30, 31, 70, 71, 73, 74, 76, 77, 79, 80, 82, 85, 86, 88, 89, 91, 92, 94, 95, 97, 98))

# Variables utilisees pour l'AMC sans les quali sup focus sur TH/CM
colonnes_combos_focus_TM <- which(seq_along(Data_AMC_TM) %in% c(6, 70, 71, 73, 74, 76, 77, 79, 80, 82, 85, 86, 88, 89, 91, 92, 94, 95, 97, 98))

#AMC sans les ES
mca.combos= MCA(Data_AMC_TM[colonnes_combos_TM], graph=FALSE)
fviz_mca_biplot(mca.combos)

#AMC avec les ES
mca.combos.ES = MCA(Data_AMC_TM[colonnes_combos_TM_ES],quali.sup=1:9, graph=FALSE)
fviz_mca_biplot(mca.combos.ES)

#AMC sans les ES focus sur TH/CM
...
fviz_mca_var(mca.combos, 
             col.var = rep(c("no","yes"),19), 
             title = "Graph of variables",
             repel = TRUE)
```

```{r}
print(fviz_mca_biplot(mca.combos.focus, invisible=c("var"), habillage = 1, repel = FALSE))
```


```{r}
fviz_cos2(mca.combos, choice = "var", axes = 1:2)
```
```{r}
for (i in 1:9){
  print(fviz_mca_biplot(mca.combos.ES, invisible=c("var","quali.sup"), habillage = i, repel = FALSE))
}
```
...
colonnes_combos_TH <- which(seq_along(Data_AMC_TH) %in% c(32, 36, 40, 44, 48, 52, 56, 60, 64, 70, 73, 76, 79, 82, 85, 88, 91, 94, 97))

#AMC avec quali.sup = combos
mca.combos.TH = MCA(Data_AMC_TH[colonnes_combos_TH], graph=FALSE)
fviz_mca_biplot(mca.combos.TH)
```
```{r}
for (i in 1:9){
  print(fviz_mca_biplot(mca.combos.TH, invisible=c("var","quali.sup"), habillage = i, repel = FALSE)) # , axes = c(3, 4) pour changer les dimensions
}
```
...
colonnes_combos_D <- which(seq_along(Data_AMC_test) %in% c(70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108))
...
mca.combos.D = MCA(Data_AMC_test[colonnes_combos_D], quali.sup=29:38, graph=FALSE)
fviz_mca_biplot(mca.combos.D)

#AMC avec quali sup douleur
mca.combos.TH.D = MCA(Data_AMC_TH[colonnes_combos_TH_D], graph=FALSE, quali.sup=11:20) #
fviz_mca_biplot(mca.combos.TH.D)

#AMC avec quali sup douleur
mca.combos.CM.D = MCA(Data_AMC_CM[colonnes_combos_CM_D], quali.sup=10:18, graph=FALSE)
fviz_mca_biplot(mca.combos.CM.D)

#AMC avec quali sup douleur
mca.combos.NM.D = MCA(Data_AMC_NM[colonnes_combos_NM_D], quali.sup=10:18, graph=FALSE)
fviz_mca_biplot(mca.combos.NM.D)
```
```{r}
# General
for (i in 29:38){
  print(fviz_mca_biplot(mca.combos.D, invisible=c("var","quali.sup"), habillage = i, repel = FALSE)) # , axes = c(3, 4) pour changer les dimensions
}
```
...
# Pour TH
for (i in 11:20){
  print(fviz_mca_biplot(mca.combos.TH.D, invisible=c("var","quali.sup"), habillage = i, repel = FALSE)) # , axes = c(3, 4) pour changer les dimensions
}
```
...
# Pour CM
for (i in 10:18){
  print(fviz_mca_biplot(mca.combos.CM.D, invisible=c("var","quali.sup"), habillage = i, repel = FALSE)) # , axes = c(3, 4) pour changer les dimensions
}
```
...
# Pour NM
for (i in 10:18){
  print(fviz_mca_biplot(mca.combos.NM.D, invisible=c("var","quali.sup"), habillage = i, repel = FALSE)) # , axes = c(3, 4) pour changer les dimensions
}
```


### ACM de TH/CM et habillages selon apparition d'ES ###

```{r}
# Variables utilisees pour l'AMC avec les quali sup ES
colonnes_TM_ES <- which(seq_along(Data_AMC_TM) %in% c(10, 70, 71, 73, 74, 76, 77, 79, 80, 82, 85, 86, 88, 89, 91, 92, 94, 95, 97, 98))
...
```
```{r}
print(fviz_mca_biplot(mca.TM.ES, invisible=c("var","quali.sup"), habillage = 1, repel = FALSE))
```
### ACM des effets secondaires et habillages selon le TM ###

```{r}
# Variables utilisees pour l'AMC avec les quali sup ES
colonnes_ES <- which(seq_along(Data_AMC_TM) %in% c(10, 11, 14, 17, 20, 23, 26, 29, 30, 31))
...
```{r}
print(fviz_mca_biplot(mca.ES, invisible=c("var","quali.sup"), habillage = 1, repel = FALSE))
```


```{r}
for (i in 2:10){
  print(fviz_mca_biplot(mca.ES, invisible=c("var","quali.sup"), habillage = i, repel = FALSE))
}
```
