...
colonnes_Mobilite <- which(seq_along(Data_AMC_Mobilite) %in% c(7,34,99))

mca.Mobilite = MCA(Data_AMC_Mobilite[colonnes_Mobilite], graph=FALSE)
...
for (var in c("D_Mob")){
  ...
  print(
    ggplot(df_Mob_D, aes(x = `Dim 1`, y = `Dim 2`, color = df_Mob_D[[var]], label = rownames(df_Mob_D))) +
      geom_point(size=3) +
...
mca.Nutrition = MCA(Data_AMC_Nutrition[colonnes_Nutrition], graph=FALSE)
...
mca.Phyto = MCA(Data_AMC_Phyto[colonnes_Phyto], graph=FALSE)
...
mca.Bouillotte = MCA(Data_AMC_Bouillotte[colonnes_Bouillotte], graph=FALSE)
