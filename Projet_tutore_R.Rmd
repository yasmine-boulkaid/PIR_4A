...
test_khi_deux <- chisq.test(table(df_age_counts$age, df_age_counts_bis$age[19:54]))

# Afficher les résultats du test
print(test_khi_deux)

####### vous avez aussi un warning "Chi-squared approximation may be incorrect" ??
```
...
table_contingence_prise_douleur <- table(data1$Prise_TH[data1$Prise_TH != "0"], data1$Douleur_TH[data1$Douleur_TH != "0"])

options(repr.plot.width = 8, repr.plot.height = 10)

# Création du graphique de mosaïque
mosaicplot(table_contingence_prise_douleur, main = "Graphique de Mosaïque", cex.axis = 0.7, col = terrain.colors(length(unique(data1$Douleur_TH))),las=4)

```

Pas ultra pertienent : trop de prises continues en proportion pour être significatif.
...
table_contingence_duree_douleur <- table(Duree_TH, data1$Douleur_TH[data1$Douleur_TH != "0"])

options(repr.plot.width = 12, repr.plot.height = 8)

# Création du graphique de mosaïque
mosaicplot(table_contingence_duree_douleur[c(4, 2, 3, 1, 5),c(1, 4, 5, 3, 2)], main = "Graphique de Mosaïque", cex.axis = 1, col = terrain.colors(length(unique(data1$Douleur_TH))),las=4)
...
table_contingence_duree_evol <- table(Duree_TH, data1$Evolution_TH[data1$Evolution_TH != "0"])

options(repr.plot.width = 12, repr.plot.height = 8)

# Création du graphique de mosaïque
mosaicplot(table_contingence_duree_evol[c(4, 2, 3, 1, 5),c(1, 4, 6, 7, 5, 3, 2)], main = "Graphique de Mosaïque", cex.axis = 0.8, col = terrain.colors(length(unique(data1$Evolution_TH))),las = 4)
...
table_contingence_tabac_douleur <- table(data$Tabac[which(data$TH == 1)], data1$Douleur_TH[data1$Douleur_TH != "0"])

mosaicplot(table_contingence_tabac_douleur[,c(1, 4, 5, 3, 2)], main = "Graphique de Mosaïque", cex.axis = 1, col = terrain.colors(length(unique(data1$Douleur_TH))),las=4)
```
...
table_contingence_alcool_douleur <- table(data$Alcool[which(data$TH == 1)], data1$Douleur_TH[data1$Douleur_TH != "0"])

mosaicplot(table_contingence_alcool_douleur[,c(1, 4, 5, 3, 2)], main = "Graphique de Mosaïque", cex.axis = 1, col = terrain.colors(length(unique(data1$Douleur_TH))),las=4)
```
...
Attention niveau interpretation : acupunture à 100% car peu de cas relevé
